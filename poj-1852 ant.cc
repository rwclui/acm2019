#include <stdio.h>
//c: number of test cases; L: the length of the pole; n: number of ants on the pole
...
main(){
   ...
   while (c-- && (2 == scanf("%d%d",&L,&n))) {//Input the length of the pole and the number of ants on the pole
      ...
      for (i=0;i<n;i++) {         // Input original positions for all ants and adjust times
         scanf("%d",&k);
         if (k > big) big = k;            // adjust the latest possible time
         if (L-k > big) big = L-k;
         if (k > L-k) k = L-k;            // adjust the earliest possible time
